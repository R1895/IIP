...
<template>
  <div class="app-container">
    ...
    >
      <el-table-column align="center" label="产线类型">
        <template slot-scope="scope">
          {{ scope.row.task_type }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="产线所含工序数量">
        <template slot-scope="scope">
          {{ scope.row.tasks != null ? scope.row.tasks.length : "暂无工序" }}
        </template>
      </el-table-column>
      <el-table-column label="产线类型描述" align="center">
        <template slot-scope="scope">
    ...
    <el-dialog width="30%" title="添加产线类型" :visible.sync="addColVisible">
      <el-form :model="newCol">
        <el-form-item label="产线类型名称">
          <el-input v-model="newCol.task_type" autocomplete="off"></el-input>
        </el-form-item>
    ...
    >
      <el-form :model="updateColData">
        ...
        <el-form-item label="车间ID">
          <el-input
            v-model="updateColData.workshop_id"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item label="产线类型值">
...
import {
  getTaskTypeManagerCols,
  addTaskTypeManagerCol,
  updateTaskTypeManagerCol,
  deleteTaskTypeManagerCol,
} from "@/api/industry";
  ...
  methods: {
    ...
    deleteCol(id) {
      this.$confirm("确定删除此行信息吗(删除后不可恢复)?", "注意", {
        ...
        .then(async () => {
          console.log("confirm");
          let res = await deleteTaskTypeManagerCol({ id: id });
          console.log(res);
    ...
    async search() {
      ...
      let res = await getTaskTypeManagerCols({ id: this.searchBox.id });
    ...
    addCol() {
      ...
      this.$confirm("确定添加新产线类型吗", "提示", {
        confirmButtonText: "确认",
        cancelButtonText: "取消",
        type: "primary",
      })
        .then(async () => {
          let res = await addTaskTypeManagerCol(data);
          if (res.data.status == 200) {
            this.$message({
              type: "success",
              message: "添加成功!",
    ...
    updateCol() {
      ...
      this.$confirm("确定更新此产线类型信息吗?", "注意", {
        ...
        .then(async () => {
          ...
          let res = await updateTaskTypeManagerCol(sendP);
          if (res.data.status == 200) {
            this.$message({
              type: "success",
              message: "更新成功!",
    ...
    fetchData(start = 1, limit = 10) {
      this.listLoading = true;
      getTaskTypeManagerCols({ start: start, limit: limit }).then(
        (response) => {
