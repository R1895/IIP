<template>
  <div class="app-container">
    <el-form :inline="true" :model="searchBox" class="demo-form-inline">
        <el-form-item label="产线状态名">
            <el-input v-model="searchBox.task_status" placeholder="产线状态名"></el-input>
        </el-form-item>
        <el-form-item>
            <el-button type="warning" @click="search">搜索</el-button>
        </el-form-item>
        <el-form-item>
            <el-button type="primary" @click="addColVisible = true">添加产线状态</el-button>
        </el-form-item>
    </el-form>
    <el-table
    ...
    >
    <el-table-column align="center" label="产线状态ID">
        <template slot-scope="scope">
          {{ scope.row.id }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="产线状态名">
        <template slot-scope="scope">
          {{ scope.row.task_status }}
        </template>
      </el-table-column>
       <el-table-column label="此状态下的产线数量" align="center">
        <template slot-scope="scope">
          {{ scope.row.tasks==null?"暂无分配":scope.row.tasks.length}}
        </template>
      </el-table-column>
    ...
    <el-dialog width="30%" title="添加产线状态" :visible.sync="addColVisible">
        <el-form :model="newCol">
            <el-form-item label="产线状态名" >
                <el-input v-model="newCol.task_status" autocomplete="off"></el-input>
            </el-form-item>
    ...
    <el-dialog width="30%" title="更新产线状态信息" :visible.sync="updateColVisible">
        <el-form :model="updateColData">
            <el-form-item label="产线状态ID" >
                <el-input v-model="updateColData.id" autocomplete="off" disabled></el-input>
            </el-form-item>
            <el-form-item label="产线状态名" >
                <el-input v-model="updateColData.task_status" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item label="产线状态描述" >
...
import { getTaskStatusManagerCols, addTaskStatusManagerCol, updateTaskStatusManagerCol, deleteTaskStatusManagerCol} from '@/api/industry'
  ...
  methods: {
    ...
    deleteCol(id){
        this.$confirm('确定删除此行信息吗(删除后不可恢复)?', '注意', {
            ...
            console.log("confirm")
            let res = await deleteTaskStatusManagerCol({id:id});
            console.log(res)
    ...
    async search(){
        ...
        sendParams.task_status = this.searchBox.task_status
        console.log(sendParams)
        let res = await getTaskStatusManagerCols(sendParams)
    ...
    addCol(){
        ...
        this.$confirm('确定添加新产线状态吗', '提示', {
            confirmButtonText: '确认',
            cancelButtonText: '取消',
            type: 'primary'
        }).then(async () => {
            let res = await addTaskStatusManagerCol(data);
            if(res.data.status==200){
                this.$message({
                    type: 'success',
                    message: '添加成功!'
    ...
    updateCol(){
        ...
        this.$confirm('确定更新此产线状态信息吗?', '注意', {
            ...
            let sendP= {
                ...
                task_status:this.updateColData.task_status,
                description:this.updateColData.description
            }
            let res = await updateTaskStatusManagerCol(sendP);
            if(res.data.status==200){
                this.$message({
                    type: 'success',
                    message: '更新成功!'
    ...
    fetchData(start=1,limit=10) {
      this.listLoading = true
      getTaskStatusManagerCols({start:start,limit:limit}).then(response => {
