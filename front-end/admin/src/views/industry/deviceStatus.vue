<template>
  <div class="app-container">
    ...
    >
      <el-table-column align="center" label="设备类别ID">
        <template slot-scope="scope">
          {{ scope.row.id }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="设备名字">
        <template slot-scope="scope">
      ...
      <el-table-column label="此类别下的机器数" align="center">
        <template slot-scope="scope">
          {{ scope.row.machine==null?0:scope.row.machine.length }}
        </template>
      </el-table-column>
    ...
    <el-dialog width="30%" title="上报设备类别" :visible.sync="addColVisible">
      <el-form :model="newCol">
        <el-form-item label="状态名">
          <el-input v-model="newCol.status_name" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="状态类别">
          <el-input v-model="newCol.status_type" autocomplete="off"></el-input>
        </el-form-item>
    ...
    >
      <el-form :model="updateColData">
        ...
        <el-form-item label="状态名">
          <el-input
            v-model="updateColData.status_name"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item label="描述">
...
import {
  ...
  addMachineStatusManagerCol,
  updateMachineStatusManagerCol,
  deleteMachineStatusManagerCol,
} from "@/api/industry";
  ...
  methods: {
    ...
    deleteCol(id) {
      this.$confirm("确定删除此行信息吗(删除后不可恢复)?", "注意", {
        ...
        .then(async () => {
          console.log("confirm");
          let res = await deleteMachineStatusManagerCol({ id: id });
          console.log(res);
    ...
    async search() {
      ...
      let res = await getMachineStatusManagerCols({ id: this.searchBox.id });
    ...
    addCol() {
      ...
      if(parseInt(this.newCol.status_type)==NaN){
        this.$message({
              type: "warning",
              message: "状态类别需为整数类型",
            });
      ...
      this.$confirm("确定添加新设备类别吗", "提示", {
        confirmButtonText: "确认",
        cancelButtonText: "取消",
        type: "primary",
      })
        .then(async () => {
          let res = await addMachineStatusManagerCol(data);
          if (res.data.status == 200) {
            this.$message({
              type: "success",
              message: "添加成功!",
      ...
      this.$confirm("确定更新此设备类别信息吗?", "注意", {
        ...
        .then(async () => {
          let sendP = {
            ...
            status_name: this.updateColData.status_name,
            description: this.updateColData.description,
          };
          let res = await updateMachineStatusManagerCol(sendP);
          if (res.data.status == 200) {
            this.$message({
              type: "success",
              message: "更新成功!",
    ...
    fetchData(start = 1, limit = 10) {
      this.listLoading = true;
      getMachineStatusManagerCols({ start: start, limit: limit }).then(
        (response) => {
