<template>
  <div class="app-container">
    <el-form :inline="true" :model="searchBox" class="demo-form-inline">
        <el-form-item label="事故ID">
            <el-input v-model="searchBox.id" placeholder="ID"></el-input>
        </el-form-item>
        <el-form-item label="车间ID">
            <el-input v-model="searchBox.workshop_id" placeholder="ID"></el-input>
        </el-form-item>
        <el-form-item>
            <el-button type="warning" @click="search">搜索</el-button>
        </el-form-item>
        <el-form-item>
            <el-button type="primary" @click="addColVisible = true">添加事故</el-button>
        </el-form-item>
    </el-form>
    <el-table
    ...
    >
    <el-table-column align="center" label="事故ID">
        <template slot-scope="scope">
          {{ scope.row.id }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="事故车间ID">
        <template slot-scope="scope">
      ...
      <el-table-column label="事故值" align="center">
        <template slot-scope="scope">
          {{ scope.row.value}}
        </template>
      </el-table-column>
    ...
    <el-dialog width="30%" title="更新事故信息" :visible.sync="updateColVisible">
...
import { getSaferateManagerCols, addSaferateManagerCol, updateSaferateManagerCol, deleteSaferateManagerCol} from '@/api/environment'
  ...
  methods: {
    ...
    deleteCol(id){
        this.$confirm('确定删除此行信息吗(删除后不可恢复)?', '注意', {
            ...
            console.log("confirm")
            let res = await deleteSaferateManagerCol({id:id});
            console.log(res)
    ...
    async search(){
        ...
        let res = await getSaferateManagerCols({id:this.searchBox.id})
    ...
    addCol(){
        ...
        this.$confirm('确定添加新事故吗', '提示', {
            confirmButtonText: '确认',
            cancelButtonText: '取消',
            type: 'primary'
        }).then(async () => {
            let res = await addSaferateManagerCol(data);
            if(res.data.status==200){
                this.$message({
                    type: 'success',
                    message: '添加成功!'
    ...
    updateCol(){
        ...
        this.$confirm('确定更新此事故信息吗?', '注意', {
            ...
            let res = await updateSaferateManagerCol(sendP);
            if(res.data.status==200){
                this.$message({
                    type: 'success',
                    message: '更新成功!'
    ...
    fetchData(start=1,limit=10) {
      this.listLoading = true
      getSaferateManagerCols({start:start,limit:limit}).then(response => {
