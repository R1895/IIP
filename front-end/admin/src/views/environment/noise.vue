<template>
  <div class="app-container">
    <el-form :inline="true" :model="searchBox" class="demo-form-inline">
        ...
        <el-form-item>
            <el-button type="primary" @click="addColVisible = true">添加噪音</el-button>
        </el-form-item>
    </el-form>
    <el-table
    ...
    >
    <el-table-column align="center" label="噪音ID">
      ...
      <el-table-column label="噪音值" align="center">
        <template slot-scope="scope">
          {{ scope.row.value}}
        </template>
      </el-table-column> 
    ...
    <el-dialog width="30%" title="上报噪音" :visible.sync="addColVisible">
    ...
    <el-dialog width="30%" title="更新噪音信息" :visible.sync="updateColVisible">
...
import { getNoiseManagerCols, addNoiseManagerCol, updateNoiseManagerCol, deleteNoiseManagerCol} from '@/api/environment'
  ...
  methods: {
    ...
    deleteCol(id){
        this.$confirm('确定删除此行信息吗(删除后不可恢复)?', '注意', {
            ...
            console.log("confirm")
            let res = await deleteNoiseManagerCol({id:id});
            console.log(res)
    ...
    async search(){
        ...
        let res = await getNoiseManagerCols({id:this.searchBox.id})
    ...
    addCol(){
        ...
        this.$confirm('确定添加新噪音吗', '提示', {
            confirmButtonText: '确认',
            cancelButtonText: '取消',
            type: 'primary'
        }).then(async () => {
            let res = await addNoiseManagerCol(data);
            if(res.data.status==200){
                this.$message({
                    type: 'success',
                    message: '添加成功!'
    ...
    updateCol(){
        ...
        this.$confirm('确定更新此噪音信息吗?', '注意', {
            ...
            let res = await updateNoiseManagerCol(sendP);
            if(res.data.status==200){
                this.$message({
                    type: 'success',
                    message: '更新成功!'
    ...
    fetchData(start=1,limit=10) {
      this.listLoading = true
      getNoiseManagerCols({start:start,limit:limit}).then(response => {
